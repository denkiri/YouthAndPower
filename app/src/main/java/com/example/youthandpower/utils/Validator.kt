...
class Validator {
    companion object {
        ...
        private val INVOICE_VALIDATION_MSG = "Enter a valid Invoice Number"
        ...
        fun isValidInvoice(data: Any, updateUI: Boolean = true): Boolean {
            val str = getText(data)
            val valid = str.trim().length > 2

            // Set error if required
            if (updateUI) {
                val error: String? = if (valid) null else INVOICE_VALIDATION_MSG
                setError(data, error)
            }
